 ...
 */
function wiloke_export_wp( $args = array() ) {
    global $wpdb, $post;

    $defaults = array( 'content' => 'all', 'author' => false, 'category' => false,
        'start_date' => false, 'end_date' => false, 'status' => false, 'terms' => false
    );
    ...
    }else if ('nav_menu_item'==$args['content']){
	    $where = $wpdb->prepare( "{$wpdb->posts}.post_type = %s AND {$wpdb->posts}.comment_status=%s AND {$wpdb->posts}.post_status=%s", 'nav_menu_item1', 'closed', 'publish' );
    }elseif ('taxonomy' == $args['content']) {
        $args['terms'] = implode(",", $args['terms']);
        $join = "INNER JOIN {$wpdb->term_relationships} ON ({$wpdb->posts}.ID = {$wpdb->term_relationships}.object_id)";
        $where = "$wpdb->term_relationships.term_taxonomy_id" . " IN(".$args['terms'].")";
    }elseif ('nav_menu_tax' == $args['content']){
    ...
    // @wiloke added page id specific
    if ('page' == $args['content']) {
        if (!empty($args['specify_page'])) {
            $where .= $wpdb->prepare(" AND {$wpdb->posts}.ID = %d", $args['specify_page']);
        }
    }

    // Grab a snapshot of post IDs, just in case it changes during the export.
    $post_ids = [];
    if (isset($args['navigation_taxonomy']) && $args['content'] == 'nav_menu_item') {
        $aArgs  = array(
            'numberposts' => -1,
            ...
            'tax_query' => array(
                array(
                    'taxonomy' => 'nav_menu',
                    'field' => 'term_id',
                    'terms' => $args['navigation_taxonomy']
                )
            )
        );
    ...
    if ( !empty($post_ids) )
    {
        foreach ( $post_ids as $postID )
        {
            ...
            {
                $aThumbnailIDs[] = get_post_thumbnail_id($postID);
            }
            ...
            if ( !empty($content) )
            {
                $aMediaIds = wiloke_get_media_ids_in_shortcodes($content);

                if ( !empty($aMediaIds) )
                {
                    $aMediaIds = explode(",", $aMediaIds);
                    $aThumbnailIDs = isset($aThumbnailIDs) && !empty($aThumbnailIDs) ? array_merge($aThumbnailIDs, $aMediaIds) :  $aMediaIds;
                }
            }
    ...
    if ( isset($aThumbnailIDs) && !empty($aThumbnailIDs) )
    {
        $post_ids = (array)$post_ids;
        $post_ids = array_merge($post_ids, $aThumbnailIDs);
    }
    ...
    if ( isset( $term ) && $term ) {
        ...
        if ( isset($args['terms']) && !empty($args['terms']) && isset($args['taxonomy']) && !empty($args['taxonomy']) )
        {
            if ( !is_array($args['terms']) )
            {
                $args['terms'] = explode(",", $args['terms']);
            }

            $custom_terms = (array) get_terms( array( 'taxonomy' => $args['taxonomy'], 'include' => $args['terms']) );
     ...
     * @param $iTermID
     */
    function wxr_nav_menu_terms($iTermID=[]) {
        $nav_menus = wp_get_nav_menus();
        if ( empty( $nav_menus ) || ! is_array( $nav_menus ) )
            return;

        foreach ( $nav_menus as $menu ) {
            if(!empty($iTermID)&&$menu->term_id!==abs($iTermID)){
                continue;
            }
            echo "\t<wp:term>";
    ...
    >
        ...
        <channel>
            ...
            <?php foreach ( $cats as $c ) : ?>
                <wp:category><wp:term_id><?php echo intval( $c->term_id ); ?></wp:term_id><wp:category_nicename><?php echo wxr_cdata( $c->slug ); ?></wp:category_nicename><wp:category_parent><?php echo wxr_cdata( $c->parent ? $cats[$c->parent]->slug : '' ); ?></wp:category_parent><?php wxr_cat_name( $c ); ?><?php wxr_category_description( $c ); ?>
                    <wp:termmeta>
                        <wp:meta_key><?php echo wxr_cdata(hsBlogCorePrefix('featured_img'));?></wp:meta_key>
                        <wp:meta_value><?php echo wxr_cdata( Term::getTermMeta(intval( $c->term_id ),hsBlogCorePrefix('featured_img')) );
                        ?></wp:meta_value>
                    </wp:termmeta>
                    <wp:termmeta>
            ...
            <?php if ( 'nav_menu_tax' == $args['content'] ) { wxr_nav_menu_terms(); } ?>
            <?php if ( 'nav_menu_item' == $args['content'] ) { wxr_nav_menu_terms($args['navigation_taxonomy']); } ?>
